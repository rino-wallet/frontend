...
describe("SubaddressListSlice", () => {
  ...
  it("fetchSubaddresses should get subaddresses list and update entities array", async() => {
    ...
    expect(walletApi.fetchWalletSubaddresses.withArgs("pony", { limit: ITEMS_PER_PAGE, offset: 1 }).callCount).to.equal(1);
    expect(store.getState().subaddressList.entities.length).to.equal(ITEMS_PER_PAGE);
    expect(store.getState().subaddressList.count).to.equal(fetchSubaddressesResponse.count - 1);
    expect(store.getState().subaddressList.hasPreviousPage).to.equal(false);
