...
export async function createPDF(config: PdfDocumentConfig, data: NewWalletPDFData): Promise<void> {
  ...
  await pdf.addPage((self: PdfDocument) => {
    ...
    self.addText("If your backup document does get exposed or copied in a way that makes you uncomfortable, the best course of action is to empty the compromised wallet into another RINO wallet and discontinue use of the old wallet.", self.leftOffset, 241);
  ...
  await pdf.addPage((self: PdfDocument) => {
    ...
    self.addText("The wallet you have just created is a multisignature wallet. To access the funds, it is necessary to use both parts from this document. So you start with recovering both wallets (see above). Once you have both ready, you can spend the funds with these steps in the command line wallet:", self.leftOffset, 113);
    self.addText("1. In the user wallet, run this command:", self.leftOffset, 130);
    self.addCommand("export_multisig_info multisig_info_user", self.leftOffset + 5, 135);
    self.addText("This command creates a new file in the current folder, called multisig_info_user.", self.leftOffset + 4, 140);
    self.addText("2. In the backup wallet, run this command:", self.leftOffset, 145);
    self.addCommand("export_multisig_info multisig_info_backup", self.leftOffset + 5, 150);
    self.addText("This command creates a new file in the current folder, called multisig_info_backup.", self.leftOffset + 4, 155);
    self.addText("3. In the user wallet, run this command:", self.leftOffset, 160);
    self.addCommand("import_multisig_info multisig_info_backup", self.leftOffset + 5, 165);
    self.addText("It needs the file multisig_info_backup in the current folder.", self.leftOffset + 4, 170);
    self.addText("4. In the backup wallet, run this command:", self.leftOffset, 175);
    self.addCommand("import_multisig_info multisig_info_user", self.leftOffset + 5, 180);
    self.addText("It needs the file multisig_info_user in the current folder.", self.leftOffset + 4, 185);
    self.addText("5. In one of the wallets (doesn't matter which), run this command to send funds to the given address.", self.leftOffset, 190);
    self.addCommand("transfer <address> <amount>", self.leftOffset + 5, 195);
    self.addText("It will not send the Moneroj, yet. Instead, it will create a file containing the partially signed transaction in the current folder. The file will be named multisig_monero_tx.", self.leftOffset + 4, 200);
    self.addText("6. In the other wallet, run this command:", self.leftOffset, 210);
    self.addCommand("sign_multisig multisig_monero_tx", self.leftOffset + 5, 215);
    self.addText("It needs the file multisig_monero_tx in the current folder.", self.leftOffset + 4, 220);
    self.addText("7. In the same wallet where you've run sign_multisig, run this command:", self.leftOffset, 225);
